...
| 4 | `normalizeCategoria()` do import (sem espaço) inconsistente com `SLUG_MAP` (com espaço) para J-PREMIUM/U-UTILITARIO | `apps/frota-ops/pages/admin.js` vs. `supabase/functions/_shared/disponibilidade.ts` | **Bug latente que vai reproduzir o caso GRUPO J** na próxima importação dessas categorias |
