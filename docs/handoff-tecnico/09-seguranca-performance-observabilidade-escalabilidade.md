...
| 9 | **Múltiplas policies de INSERT coexistindo em `solicitacoes` e `solicitacao_itens`**, sendo a mais permissiva (`WITH CHECK true`) sempre vencedora | 🟡 Médio | ✅ **Corrigido em rodada posterior** (migration 023) — policies redundantes removidas |
| 10 | Policies de leitura pública (`categorias`, `protecoes`, `adicionais`, `sazonalidade`, `locais`) filtram só `ativo=true`, **sem checar tenant** | 🟡 Médio | ❌ **Não corrigido** — requer decisão de arquitetura (como resolver tenant de visitante anônimo); inofensivo hoje com 1 único tenant |
| 11 | RLS de `frota_veiculos`/`frota_reservas`/`frota_patios` permitia escrita a **qualquer usuário autenticado do tenant**, sem exigir `fn_sou_admin()` | 🟡 Médio | ✅ **Corrigido em rodada posterior** (migration 022) — policies de INSERT/UPDATE/DELETE agora exigem `fn_sou_admin()` |
| 12 | `admin-user-manager`: erro do `INSERT` em `usuarios` não verificado na ação `create` | 🟠 Alto | ✅ **Corrigido em rodada posterior** — erro verificado com rollback (`deleteUser`); `ALLOWED_ROLES` alinhado ao CHECK constraint real (removida role `'balcao'` inexistente no banco, causa raiz do bug) |
| 13 | `normalizeCategoria()` do import gera `'J-PREMIUM'`/`'U-UTILITARIO'` sem espaço, inconsistente com o `SLUG_MAP` | 🟠 Alto (funcional, não segurança) | ✅ **J-PREMIUM resolvido** (categoria eliminada, mesclada em "J"). **U-UTILITARIO intencionalmente não corrigida** — é categoria "fantasma" não-pública por decisão do dono do produto, nunca deve ser exposta via `SLUG_MAP` mesmo |
| 14 | Regra de horário de funcionamento do local não validada server-side em `criar-solicitacao` | 🟢 Baixo | ✅ **Corrigido em rodada posterior** — valida `permite_retirada/devolucao`, janela de horário, `disponivel_domingo` |
| 15 | `max_cadeirinhas` da categoria não validado server-side | 🟢 Baixo | ✅ **Corrigido em rodada posterior** — soma de adicionais `is_cadeirinha` validada contra `categorias.max_cadeirinhas` |
| 16 | TOCTOU race condition na verificação de disponibilidade (ADR-003) | 🟡 Médio | ✅ **Corrigido em rodada posterior** — `pg_advisory_xact_lock` por `(tenant_id, categoria_id)` em `inserir_solicitacao_completa` |
